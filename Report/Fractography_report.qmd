...
print(df['cross_entropy'].apply(lambda x:x<2).value_counts().rename("Cross Enrtopy<2"))
```

During segmentation, it was observed that there seemed to be an ideal size where the model could more easily segment the defects. This can be seen in the above graph where in both low and high quality samples, the larger the sample on the screen the less agreement it had with the existing mask. There are only 22 images that fail this test
...
We can see that the model seems to perform the best under ideal processing parameters, with the 
