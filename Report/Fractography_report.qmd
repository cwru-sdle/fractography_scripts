...
portion_vs_entropy_fig, (portion_vs_entropy_ax,good_portion_vs_entropy_ax,bad_portion_vs_entropy_ax) = plt.subplots(3, 1, tight_layout=True,figsize=(16,20))
portion_vs_entropy_scatter = portion_vs_entropy_ax.scatter(df['screen_portion'],df['cross_entropy'],
    c=df['energy_density'],
    cmap='inferno',  # or another colormap you prefer
    vmin=df['energy_density'].min(),
    vmax=df['energy_density'].max()/2)
portion_vs_entropy_ax.set_xlabel("Portion of Screen")
portion_vs_entropy_ax.set_ylabel("Binary Cross Entropy")
...
good_portion_vs_entropy_scatter = good_portion_vs_entropy_ax.scatter(good_df['screen_portion'],good_df['cross_entropy'],
    c=good_df['energy_density'],
    cmap='inferno',  # or another colormap you prefer
    vmin=good_df['energy_density'].min(),
    vmax=good_df['energy_density'].max()/2)
good_portion_vs_entropy_ax.set_xlabel("Portion of Screen")
good_portion_vs_entropy_ax.set_ylabel("Binary Cross Entropy")
# Compute a linear fit using numpy's polyfit
x = good_df['screen_portion'].values
y = good_df['cross_entropy'].values
...
good_portion_vs_entropy_ax.plot(x, y_pred, color='red', label=f'{round(m,1)}*x+{round(b,1)}, R^2={round(r_squared,3)}')
bad_df = df[df['cross_entropy'].apply(lambda x:x>2)].copy()
bad_portion_vs_entropy_scatter = bad_portion_vs_entropy_ax.scatter(bad_df['screen_portion'],bad_df['cross_entropy'],
    c=bad_df['energy_density'],
    cmap='inferno',  # or another colormap you prefer
    vmin=bad_df['energy_density'].min(),
    vmax=bad_df['energy_density'].max()/2)
bad_portion_vs_entropy_ax.set_xlabel("Portion of Screen")
bad_portion_vs_entropy_ax.set_ylabel("Binary Cross Entropy")
# Compute a linear fit using numpy's polyfit
x = bad_df['screen_portion'].values
y = bad_df['cross_entropy'].values
m, b = np.polyfit(x, y, 1)
y_pred = m * x + b
y_mean = np.mean(y)
ss_tot = np.sum((y - y_mean)**2)
ss_res = np.sum((y - y_pred)**2)
r_squared = 1 - (ss_res / ss_tot)
m, b = np.polyfit(bad_df['screen_portion'], bad_df['cross_entropy'], 1)
bad_portion_vs_entropy_ax.plot(x, y_pred, color='red', label=f'{round(m,1)}*x+{round(b,1)},R^2={round(r_squared,3)}')
bad_portion_vs_entropy_ax.legend()
good_portion_vs_entropy_ax.legend()
portion_vs_entropy_fig.colorbar(portion_vs_entropy_scatter, ax=portion_vs_entropy_ax, label=energy_density_label)
portion_vs_entropy_fig.colorbar(good_portion_vs_entropy_scatter, ax=good_portion_vs_entropy_ax, label=energy_density_label)
portion_vs_entropy_fig.colorbar(bad_portion_vs_entropy_scatter, ax=bad_portion_vs_entropy_ax, label=energy_density_label)
portion_vs_entropy_fig.show()
```
