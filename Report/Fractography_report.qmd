...
#| echo: false
#| fig-cap: "Shape 1 and 2 should be significantly worse stress concentration factors than shape 3 because they have sharp edges. Aspect ratio can capture the difference between shape 2 and 3, but not between shape 1 and 3. Sharpness, by contrast, can capture both differences, so should be a better predictor of fatigue performance."
knitr::opts_chunk$set(fig.width = 8, fig.height = 6)
...
```{python correlation_plot,echo=FALSE,error=FALSE,results='hide'}
combined_df = pd.read_csv('/mnt/vstor/CSE_MSE_RXF131/lab-staging/mds3/AdvManu/fractography/combined_df.csv')
...
results_df = initiating_defect_features.regression_on_df(df)
```
```{python naive regression prints, echo=FALSE}
print("Best Model")
print(results_df.loc[results_df["r2"].idxmax()])
print("Best Model Without Image Features")
print(results_df.loc[results_df[(results_df['aspect_ratio']==False) &(results_df['max_sharpness']==False)]["r2"].idxmax()])
```
From the above correlation plot, sharpness and aspect ratio do vary based on the portion of the screen the defect takes up. Naively correlating the image features to the mechanical properties of the sample, we get poor results, with an increase in $R^2$ of only 0.02

However, there are multiple images of the same initiating defect, and during segmentation it was observed that the quality of the mask seemed to depend on the size of the defect on the screen. The model seemed to be segmenting larger defects into different regions, with more extensive notation required to create a usable mask. Therefore, selecting only high quality masks may result in better properties.

### Validation

```{python mask validation,echo=FALSE,error=FALSE,results='hide'}
combined_df = pd.read_csv('/mnt/vstor/CSE_MSE_RXF131/lab-staging/mds3/AdvManu/fractography/combined_df.csv')
points_df = combined_df[~combined_df['points'].isna()]
df = initiating_defect_features.make_feature_df(points_df)
columns = ["screen_portion","max_sharpness","aspect_ratio","perimeter","pixels"]
...
df["inverse_cross_entropy"] = df.apply(
    lambda x: torch.nn.functional.binary_cross_entropy(
        torch.Tensor(cv2.resize(x['imgs'],(1024,1024))/255),
        torch.Tensor(initiating_defect_mask_validation.invert_mask(x['SAM_processed_output'])/255)
    )
,axis=1).apply(lambda x: x.detach().item())
df['SAM_processed_output'] = df.apply(lambda x: initiating_defect_mask_validation.invert_mask(x['SAM_processed_output']) if x['cross_entropy']>x["inverse_cross_entropy"] else x['SAM_processed_output'],axis=1)
...
This was done by using the center of mass as a label, the selecting the largest defect in the image, then using the binary cross entropy between these two images as a metric for the agreement between these images. This will measure how well the model performs for that defects, since it compares the mask created with full human supervision and the mask created with minimal supervision.

From the above histogram of results, we can see the majority of samples are high quality, with the higher the binary cross entropy, the fewer samples we find.

```{python portion vs entropy, echo=FALSE,results='hide'}
...
```{python showing counts, echo=FALSE}
print(df['minimum_entropy'].apply(lambda x:x<2).value_counts().rename("Cross Enrtopy<2"))
```

During segmentation, it was observed that there seemed to be an ideal size where the model could more easily segment the defects. This can be seen in the above graph where in both low and high quality samples, the larger the sample on the screen the less agreement it had with the existing mask. When we cutoff samples with a binary cross entropy of over 2, there are only 22 images that fail this test, as seen in the above table.

```{python energy density vs entropy, echo=FALSE,results='hide'}
entropy_vs_energy_fig, entropy_vs_energy_ax = plt.subplots(1, 1, tight_layout=True)
entropy_vs_energy_scatter = entropy_vs_energy_ax.scatter(good_df['energy_density'],good_df['minimum_entropy'],
    c=good_df['energy_density'],
    cmap='inferno',  # or another colormap you prefer
...
We can see that segmentation works best with a moderate energy density. This is the region where the initiating defect is primarily gas pores, which are both significantly smaller and almost perfectly circular. Having a less complex shape likely makes them easier to segment, and they very rarely take up the majority of the screen because of the resolution limits of the microscope. Additionally, the moderate energy densities generally only had a single defect in the image, while those on the edges had many on the same image, which made the segmentation task more difficult.

```{python Regression on Features,echo=FALSE,results='hide'}
...
filter_df = df[(df['screen_portion']>initiating_defect_mask_validation.FILTER)&(df['minimum_entropy']<2)].dropna().reset_index(drop=True)
best_rows = []
...
```
```{python print validated regressions, echo=FALSE}
for idx in idxs:
    print(idx[0])
    print(filter_results_df.iloc[idx[1]])
```
Additionally, by looking at individual examples, it can be seen that a filter on lower image size is required, and a filter on rows without all necessary information is needed. We can then perform a variety of regression techniques and plot the most effective ones. Additionally, from other tests, there must be a way to filter for different images on the same sample, and it seems that the image features are most predictive on smaller images. From the above regressions, we can see that the model predicts the mean well, with the residuals averaging 0. However, when looking at the absolute of the residuals, we can, surprisingly, find that the model performs best on more extreme samples. Additionally, when the regression has access to all features, it performs the best, with an improvement from $R^2$ of 0.212 to 0.322, a 65% improvement.

```{python residual histograms,echo=FALSE}
...
for idx in idxs:
    initiating_defect_mask_validation.histogram_residuals_by_image_class(filter_df, filter_results_df, idx=idx[1])
```

We can also see that the residuals are approximately normally distributed, as would be expected.
