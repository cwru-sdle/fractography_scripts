...
def run_script(input_df):
    ...
    from ...Unet_train.unet import Unet
    import cv2
    from ...Unet_train.train_GAN import train_GAN
    from ...Unet_train.discriminator_loss import discriminator_loss
    from ...Unet_train.semi_supervised_loss import semi_supervised_loss
    from ...Unet_train.multiclass_dataset import Multiclass_dataset
    #importlib.reload(make_dataset)
    from ...Unet_train.make_dataset import dataset_setup
    import torch
    ...
    # x_unsup is the 
    segmentor = Unet(
        input_channels = input_df['initial_features'],
        output_channels=input_df['features_expanded'],
        encoder_pairs=input_df['encoder_pairs'],
        initial_features=input_df['initial_features'],
        features_expanded=input_df['features_expanded']
    ).to('cuda:0')
    discriminator = Unet(
        input_channels = input_df['initial_features'],
        output_channels=input_df['features_expanded'],
    ...
    train_GAN(
        ...
        valid_dl,
        unsegmented_raw,
        epochs=input_df['epochs'],
        accumulation_steps=input_df['accumulation_steps'],
        save_path = '/home/aml334/CSE_MSE_RXF131/cradle-members/mds3/aml334/GAN_MODELS_1/test_0',
        learning_rate=input_df['learning_rate'],
    )