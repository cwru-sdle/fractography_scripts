...
parser.add_argument("epochs_",type=int)
...
def generate_mask(img_arr, red_threshold, saturation_threshold):
    ...
    thresholded_mask = np.zeros_like(red_channel, dtype=np.uint8)

    # apply thresholding using NumPy operations
    red_threshold_mask = blue_channel > red_threshold
    saturation_mask = (green_channel!=blue_channel) & (red_channel!=blue_channel)
    thresholded_mask[red_threshold_mask & saturation_mask] = 255

    # find contours
    cnts = cv2.findContours(thresholded_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cnts = cnts[0] if len(cnts) == 2 else cnts[1]

    img_bgr = cv2.cvtColor(thresholded_mask.astype('uint8'), cv2.COLOR_GRAY2BGR)

    # fill contours with polygon
    for c in cnts:
        cv2.drawContours(img_bgr, [c], -1, (255,255,255), -1)

    # convert to binary image
    img_gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    img_binary = np.where(img_gray > 0, True, False)

    # remove small objects
    img_mask = remove_small_objects(img_binary, min_size=10)
    
    return img_mask.astype('uint8')

Resize = v2.Resize([512,512],antialias=True)
def Resize_mask(img):
    img = generate_mask(img,0,0)
    img = Resize(img)
    return img
